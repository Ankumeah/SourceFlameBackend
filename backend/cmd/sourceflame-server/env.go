...
func set_settings(s *a.Settings) {
  ...
  if err != nil { log.Fatalf("Error while parseing DATABASE_MAX_CONNS: %v\n", err.Error()) }
  s.DATABASE_MAX_IDLE_CONNS, err = strconv.Atoi(env_vars["DATABASE_MAX_IDLE_CONNS"])
  if err != nil { log.Fatalf("Error while parseing DATABASE_MAX_IDLE_CONNS: %v\n", err.Error()) }
  s.DATABASE_MAX_LIFETIME, err = time.ParseDuration(env_vars["DATABASE_MAX_LIFETIME"])
  if err != nil { log.Fatalf("Error while parseing DATABASE_MAX_LIFETIME: %v\n", err.Error()) }
  s.DATABASE_MAX_IDLE_TIME, err = time.ParseDuration(env_vars["DATABASE_MAX_IDLE_TIME"])
  if err != nil { log.Fatalf("Error while parseing DATABASE_MAX_IDLE_TIME: %v\n", err.Error()) }

  s.JWT_KEY = []byte(env_vars["JWT_KEY"])
  s.JWT_LIFESPAN, err = time.ParseDuration(env_vars["JWT_LIFESPAN"])
  if err != nil { log.Fatalf("Error while parseing JWT_LIFESPAN: %v\n", err.Error()) }

  s.TOKEN_TIMEOUT, err = time.ParseDuration(env_vars["TOKEN_TIMEOUT"])
  if err != nil { log.Fatalf("Error while parseing TOKEN_TIMEOUT: %v\n", err.Error()) }
  s.TOKEN_LIMIT, err = strconv.Atoi(env_vars["TOKEN_LIMIT"])
  if err != nil { log.Fatalf("Error while parseing TOKEN_LIMIT: %v\n", err.Error()) }
}
