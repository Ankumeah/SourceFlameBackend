...
func repos(r *gin.RouterGroup, app *a.App) {
	...
	group.POST("/members/:member", func(c *gin.Context) {
		ctx := c.Request.Context()
		userId := c.GetUint64(middlewares.UserIdField)
		repoName := c.Param("repo_name")
		...
		memberId, ok := getUserId(c, app.UserDb, member)
		if !ok {
			return
		}

		err := app.GitDb.AddMember(ctx, repoId, memberId)
		if errors.Is(err, database.ErrSafe) {
			c.JSON(badRequest(err))
			return
		} else if err != nil {
	...
	group.DELETE("/members/:member", func(c *gin.Context) {
		ctx := c.Request.Context()
		userId := c.GetUint64(middlewares.UserIdField)
		repoName := c.Param("repo_name")
		member := c.Param("member")
		...
		err := app.GitDb.RemoveMember(ctx, repoId, memberId)
		if errors.Is(err, database.ErrSafe) {
			c.JSON(badRequest(err))
			return
		} else if err != nil {
